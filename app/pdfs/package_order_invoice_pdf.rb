class PackageOrderInvoicePdf < Prawn::Document
  
  def initialize(params ={})
    super(:top_margin => 25)
    font_type = {:font_big => 23, :font_medium => 12, :font_small => 9}
    po = PackageOrder.find params[:id]
    render_content(font_type, po)

  end
    
  private
  
  def render_content(font, po)
    ...
    rows << ["", "Tagihan", "", ""]
    rows << ["","Customer: #{po.buyer_name}", "Nomor Tagihan: #{po.invoice_number}",""]
    rows << ["","Particulars", "Jumlah",""]

    rows << ["", po.package.title, po.budget_display, ""]
    if po.promotion.present?
      promotion = Promotion.find(po.promotion)
      rows << ["","Discount", "#{po.currency.code} #{promotion.voucher_value_display(po)}",""]
    end
    
    rows << ["","Kode Bayar", po.payment_code_display, ""]
    rows << ["","Total", po.total_cost_display,""]
    ...
    stamp_at "paid", [480, 680] if po.order_status_id == OrderStatus.get_paid.id
