class ServiceOrderInvoicePdf < Prawn::Document
  
  def initialize(params ={})
    super(:top_margin => 25)
    font_type = {:font_big => 23, :font_medium => 12, :font_small => 9}
    jo = ServiceOrder.find params[:id]
    render_content(font_type, jo)

    if params[:for_attachment]
      # cek direktori, kalo belum ada dibuatin
      invoice_directory = "#{Rails.root.to_s}/public/assets/media/invoice/"
      FileUtils.mkdir_p invoice_directory unless File.directory? invoice_directory
      
      render_file(invoice_directory + "file.pdf")
    end
  end
  ...
  def render_content(font, jo)
    ...
    rows << ["", "Tagihan", "", ""]
    rows << ["","Freelancer: #{jo.freelancer.username}", "Tanggal: #{jo.created_at.strftime('%d %b %Y')}",""]
    rows << ["","Employer: #{jo.employer.username}", "Nomor Tagihan: #{jo.invoice_number}",""]
    rows << ["","Particulars", "Jumlah",""]
    rows << ["",jo.service.title , jo.budget_display, ""]
